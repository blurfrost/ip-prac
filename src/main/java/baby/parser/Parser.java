...
public class Parser {
    ...
    public static Command parse(String input) {
        if (input == null || input.trim().isEmpty()) {
            throw new exception.InvalidCommandException(input);
        }
        ...
        if (trimmedInput.equals("bye")) {
            ...
            try {
                ...
                throw new exception.InvalidIndexException(indexStr, 1, 1);
            }
    ...
    private static baby.command.AddDeadlineCommand parseDeadlineInput(String input) {
        ...
        if (slashIndex > 0) {
            ...
            return new baby.command.AddDeadlineCommand(description, parsedDate);
        } else {
            throw new exception.InvalidTaskFormatException(TaskType.DEADLINE.getUsageMessage(), fullInput, "Example: deadline homework /by Sunday");
        }
    }
    
    private static baby.command.AddEventCommand parseEventInput(String input) {
        ...
        if (fromIndex > 0 && toIndex > fromIndex) {
            ...
            return new baby.command.AddEventCommand(description, parsedFrom, parsedTo);
        } else {
            throw new exception.InvalidTaskFormatException(TaskType.EVENT.getUsageMessage(), fullInput, "Example: event meeting /from Mon 2pm /to 4pm");
        }
    }
}
